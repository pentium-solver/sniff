...
func initBaseDir() {
	...
	// Fallback: walk up until we find the library/ directory (repo root).
	sentinel := filepath.Join("library", "universal", "frida_universal_unpin.js")
	if _, err := os.Stat(filepath.Join(baseDir, sentinel)); err != nil {
		cwd, _ := os.Getwd()
		for _, candidate := range []string{cwd, filepath.Dir(cwd), filepath.Dir(filepath.Dir(cwd))} {
			if _, err := os.Stat(filepath.Join(candidate, sentinel)); err == nil {
				baseDir = candidate
				break
			}
		}
...
func fridaScripts() []fridaScript {
	u := func(n string) string { return scriptPath(filepath.Join("library", "universal", n)) }
	a := func(n string) string { return scriptPath(filepath.Join("library", "app-specific", n)) }
	return []fridaScript{
		{"universal", "Universal SSL Unpin", "BEST", "green",
			u("frida_universal_unpin.js"),
...
func pilotSignupHandoffScript() fridaScript {
	return fridaScript{
		ID: "pilot_fj_signup_handoff", Name: "Pilot Signup Handoff",
		Label: "HANDOFF", LabelColor: "red",
		Path: scriptPath(filepath.Join("library", "app-specific", "pilot_fj_signup_handoff.js")),
		Desc: "Pilot-specific signup handoff.",
	}
}

func linkedInCronetScript() fridaScript {
	return fridaScript{
		ID: "linkedin_cronet", Name: "LinkedIn Cronet Patch",
		Label: "LINKEDIN", LabelColor: "blue",
		Path: scriptPath(filepath.Join("library", "app-specific", "linkedin_cronet_patch.js")),
		Desc: "Spawn-time LinkedIn Cronet patch: disable QUIC, DNS remap, stale-DNS treatment.",
	}
}

func linkedInChallengeTraceScript() fridaScript {
	return fridaScript{
		ID: "linkedin_challenge_trace", Name: "LinkedIn Challenge Trace",
		Label: "LI-REPLAY", LabelColor: "magenta",
		Path: scriptPath(filepath.Join("library", "app-specific", "linkedin_challenge_trace.js")),
		Desc: "Hooks LinkedIn auth challenge flow.",
	}
}

func dailyPayScript() fridaScript {
	return fridaScript{
		ID: "dailypay", Name: "DailyPay SSL Bypass",
		Label: "DAILYPAY", LabelColor: "red",
		Path: scriptPath(filepath.Join("library", "app-specific", "dailypay_bypass.js")),
...
func speedwayScript() fridaScript {
	return fridaScript{
		ID: "speedway", Name: "Speedway SSL Bypass",
		Label: "SPEEDWAY", LabelColor: "red",
		Path: scriptPath(filepath.Join("library", "app-specific", "speedway_bypass.js")),
		Desc: "Speedway/7-Eleven-specific OkHttp3 + TrustManagerImpl + DataTheorem bypass.",
	}
}

func papajohnsScript() fridaScript {
	return fridaScript{
		ID: "papajohns", Name: "Papa Johns Flutter Bypass",
		Label: "PAPAJOHNS", LabelColor: "red",
		Path: scriptPath(filepath.Join("library", "app-specific", "papajohns_bypass.js")),
		Desc: "Papa Johns Flutter: BoringSSL native patch + Java TrustManager fallback.",
	}
}
...
func adbShell(cmd string) string {
	...
	result := strings.TrimSpace(strings.ReplaceAll(string(out), "\r", ""))
	debugLog("adb shell %q → %q (err=%v, %dms)", cmd, truncate(result, 80), err, time.Since(t0).Milliseconds())
	return result
}
...
func adbRun(shellCmd string) string {
	t0 := time.Now()
	result := runCmd("adb", "shell", shellCmd)
	debugLog("adbRun %q → %q (%dms)", shellCmd, truncate(result, 80), time.Since(t0).Milliseconds())
	return result
}
...
// adbPing is the tight-timeout variant used by the ping handler.
// Returns: (connected, human detail, raw `adb devices` output).
// Uses a dedicated 4-second context so it never drifts to 10 s under load.
func adbPing() (bool, string, string) {
	...
	if ctx.Err() == context.DeadlineExceeded {
		return false, "adb devices timed out after 4s", raw
	}
	if err != nil {
		return false, "adb error: " + err.Error(), raw
	}
	...
	for _, line := range strings.Split(raw, "\n")[1:] {
		...
		if strings.Contains(line, "\tdevice") {
			...
			if len(parts) > 0 {
				return true, "connected: " + parts[0], raw
			}
			return true, "connected", raw
		}
		// Report the specific non-ready state (offline, unauthorized, etc.)
		if strings.Contains(line, "\t") {
			fields := strings.SplitN(line, "\t", 2)
			if len(fields) == 2 {
				return false, "device " + fields[0] + " is " + fields[1], raw
			}
		}
	}

	if strings.Contains(raw, "no devices/emulators") || raw == "" || strings.TrimSpace(strings.Join(strings.Split(raw, "\n")[1:], "")) == "" {
		return false, "no devices attached", raw
	}
	return false, "unexpected adb output", raw
}
...
func fetchDeviceInfo(fridaServer string) deviceInfo {
	// All ADB calls are independent — run them concurrently so the device
	// page loads in ~max(one_call) instead of sum(all_calls).
	var (
		wg                        sync.WaitGroup
		model, android, sdk       string
		selinux, proxy, hostIP    string
		fridaRunning, isEmulator  bool
		connected                 bool
	)
	...
	run(func() { model = adbRun("getprop ro.product.model") })
	run(func() { android = adbRun("getprop ro.build.version.release") })
	run(func() { sdk = adbRun("getprop ro.build.version.sdk") })
	run(func() { selinux = adbRun("su -c 'getenforce 2>/dev/null'") })
	run(func() { fridaRunning = adbFridaRunning(fridaServer) })
	run(func() { proxy = adbGetProxy() })
	run(func() { hostIP = detectHostIP() })
	run(func() { connected = adbConnected() })
	run(func() { isEmulator = strings.TrimSpace(adbRun("getprop ro.kernel.qemu")) == "1" })
	wg.Wait()
...
// fridaDisguisedPath returns the path of the renamed frida-server binary.
// We copy the binary under an innocuous name so that security SDKs scanning
// /proc/*/cmdline for "frida" won't find our process.
func fridaDisguisedPath(serverPath string) string {
	dir := filepath.Dir(serverPath)
	return dir + "/bs_native64"
}

func adbFridaRunning(serverPath string) bool {
	// Check both the original name and the disguised name.
	for _, name := range []string{filepath.Base(serverPath), "bs_native64"} {
		out := adbRun(fmt.Sprintf("su -c 'pidof %s 2>/dev/null || echo no'", name))
		if out != "no" && out != "" {
			return true
		}
	}
...
func adbStartFridaServer(serverPath string) {
	...
	disguised := fridaDisguisedPath(serverPath)
	// Copy frida-server to a name that won't trigger "frida" string scans.
	// -nt test: only copy if source is newer than destination.
	adbShell(fmt.Sprintf(
		"su -c 'test -f %s && test ! %s -nt %s || (cp %s %s && chmod 755 %s)'",
		disguised, serverPath, disguised, serverPath, disguised, disguised,
	))
	cmd := exec.Command("adb", "shell", fmt.Sprintf("su -c '%s -D &'", disguised))
	cmd.Start()
...
// Spawn launches the app via Frida spawn mode so the script is injected before
// any app code runs. logFn (optional) receives each line of Frida output in
// real-time — use it to forward logs to the capture send() function.
func (f *fridaManager) Spawn(pkg, scriptPath, proxyHost string, proxyPort int) error {
	return f.SpawnWithLog(pkg, scriptPath, proxyHost, proxyPort, nil)
}

func (f *fridaManager) SpawnWithLog(pkg, scriptPath, proxyHost string, proxyPort int, logFn func(string)) error {
	config, _ := json.Marshal(map[string]interface{}{"host": proxyHost, "port": proxyPort})
	os.WriteFile("/tmp/frida_proxy_config.json", config, 0644)
	adbWriteFridaProxyConfig(proxyHost, proxyPort)
	...
	// Non-interactive subprocess (no TTY) → Frida auto-resumes after loading
	// the script. No --no-pause flag needed (and it doesn't exist in older builds).
...
func (m *model) captureSequenceStandard(send func(string, string), flowCh chan capturedFlow, stopCh chan struct{}, hostIP string) {
	...
	if newPID == 0 {
		// App died during the ART wait — likely detected frida-server or root
		// before our hooks could load. Retry with spawn mode: Frida launches
		// the app itself so the bypass script is active before the first
		// Java instruction, including Application.onCreate() security checks.
		send("App died during init — retrying with spawn mode", "yellow")
		adbForceStop(pkg)
		if sleepOrStop(stopCh, 500*time.Millisecond) {
			return
		}
		if err := m.frida.SpawnWithLog(pkg, script.Path, hostIP, port, func(line string) {
			send(line, "")
		}); err != nil {
			send("Spawn failed: "+err.Error(), "red")
			return
		}
		send("Frida spawning "+pkg+" (hooks pre-loaded)...", "green")
		pid = waitForPID(pkg, stopCh, 40, 300*time.Millisecond)
		if pid == 0 {
			send("App failed to spawn", "red")
			return
		}
		send(fmt.Sprintf("Spawned (pid %d) — hooks pre-loaded", pid), "green")
		m.appPID = pid
...
func main() {
	...
	for _, arg := range os.Args[1:] {
		...
		case "--web", "-web":
			webFlag = true
		case "--local", "-local":
			webFlag = true // --local implies web mode
			localFlag = true
		case "--debug", "-debug":
			debugMode = true
